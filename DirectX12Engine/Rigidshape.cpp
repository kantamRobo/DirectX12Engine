...
Rigidshape::Rigidshape()
{
	...
	//rigidbody = std::make_unique<btRigidBody>(mass, motion_state, sphere_shape, inertia);

	// ワールドに剛体オブジェクトを追加
	//g_pDynamicsWorld->addRigidBody(rigidbody.get());


	//BULLETUtiliry::ConvertbtVector3toVector3(m_transform.position, rigidbody->getCenterOfMassTransform().getOrigin());
}

void Rigidshape::Tick()
{
