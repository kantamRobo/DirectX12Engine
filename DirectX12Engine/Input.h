...
namespace Input
{
	...
	struct MouseState
	{
		...
		DirectX::XMFLOAT2 delta_position = DirectX::XMFLOAT2(0, 0);
		float delta_wheel = 0;
		float pressure = 1.0f;
	...
	{
		uint32_t buttons = 0;
		DirectX::XMFLOAT2 thumbstick_L = XMFLOAT2(0, 0);
		DirectX::XMFLOAT2 thumbstick_R = XMFLOAT2(0, 0);
		float trigger_L = 0;
		float trigger_R = 0;
	};
	...
	DirectX::XMFLOAT4 GetPointer();
	// set pointer position (eg. mouse pointer)
	void SetPointer(const DirectX::XMFLOAT4& props);
	// hide pointer
	void HidePointer(bool value);
	// read analog input from controllers, like thumbsticks or triggers
	DirectX::XMFLOAT4 GetAnalog(GAMEPAD_ANALOG analog, int playerIndex = 0);
