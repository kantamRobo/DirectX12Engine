...
int RenderPath_BindLua::GetLayerMask(lua_State* L)
{
	...
	LuaScript::SSetInt(L, *reinterpret_cast<int*>(&mask));
	return 1;
}
int RenderPath_BindLua::SetLayerMask(lua_State* L)
{
	...
	if (argc > 0)
	{
		int mask = LuaScript::SGetInt(L, 1);
		component->setlayerMask(*reinterpret_cast<uint32_t*>(&mask));
	}
	else
		LuaScript::SError(L, "SetLayerMask(uint mask) not enough arguments!");
	return 0;
}
...
void RenderPath_BindLua::Bind()
{
	...
	{
		initialized = true;
		Luna<RenderPath_BindLua>::Register(LuaScript::GetLuaState());
	}
}
